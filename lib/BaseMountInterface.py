"""
class to wrap a metaBSFS directory and provide convenience functions to get at the metadata stored there

The currently supported metadata extraction uses the .href file created by metaBSFS, but is limited to first class entities like projects and samples
...
eg. ~/BaseSpace/current_user/Projects/Hyperion\ Test/Samples/
which is the owning directory for the "Hyperion Test" samples. Should this be a directory of type "project" and id of the "Hyperion Test" project?
"""
...
class BaseMountInterface(object):
    def __init__(self, path):
        ...
        if not self.__validateBSFS__():
            raise BaseMountInterfaceException("Path: %s does not seem to be a metaBSFS path" % self.path)
        self.__resolveDetails__()

    def __validateBSFS__(self):
        """
